 ...
 * Takes a product description and founderId, calls the AI generator to produce
...
export async function POST(request: NextRequest) {
  let body: { productDescription?: string; founderId?: string };
  try {
  ...
  const founderId = body.founderId?.trim() || undefined;

  try {
    const result = await generateICPSet(desc, founderId ?? 'anonymous');
